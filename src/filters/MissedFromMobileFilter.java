...
public class MissedFromMobileFilter implements CdrFilter{
	...
	public int parse(String[] dataArray, List<String> headerColumns) {
		if (dataArray[headerColumns.indexOf(DEST_DEVICE_NAME.getCode())].startsWith("SEP")
				&& (!"Radio_Controller_1".equals(dataArray[headerColumns.indexOf("origDeviceName")])
				|| dataArray[headerColumns.indexOf("origDeviceName")] == null)
				&& Integer.parseInt(dataArray[headerColumns.indexOf("duration")]) == 0
				&& dataArray[headerColumns.indexOf("callingPartyNumber")].matches("^(06|07)\\d{8}$")
